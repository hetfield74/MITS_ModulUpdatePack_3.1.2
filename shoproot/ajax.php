<?php
/* -----------------------------------------------------------------------------------------
   $Id: ajax.php 16278 2025-01-22 16:10:24Z GTB $
...
// BOF - Fallback for shopversion < 2.0.5.0
if (!function_exists('xtc_gzip_output')) {
  require_once(DIR_FS_INC.'xtc_gzip_output.inc.php');
}
if (!function_exists('xtc_check_gzip')) {
  require_once(DIR_FS_INC.'xtc_check_gzip.inc.php');
}
// EOF - Fallback for shopversion < 2.0.5.0

// gzip compression
// BOF - Don't use GZIP compression in ajax.php in shopversion < 2.0.5.0 (includes/classes/modified_cache.php came in 2.0.5.0)
if (is_file(DIR_FS_CATALOG.'includes/classes/modified_cache.php')) {
  if (!isset($_REQUEST['speed'])
...
// EOF - Don't use GZIP compression in ajax.php in shopversion < 2.0.5.0 (includes/classes/modified_cache.php came in 2.0.5.0)
...
if (defined('STORE_PAGE_PARSE_TIME') && STORE_PAGE_PARSE_TIME == 'true') {
  ...
  if ($parse_time >= STORE_PAGE_PARSE_TIME_THRESHOLD) {
    // BOF - Fallback for shopversion < 2.0.7.0
    ...
    // EOF - Fallback for shopversion < 2.0.7.0
  }
}
