<?php
 /*-------------------------------------------------------------
  ...
  if ($order->info['orders_status'] != $status || $comments != '' || $email_preview) {
    if (!$email_preview) {  
      // BOF - Fallback for shop version < 3.0.0
      if (is_file(DIR_FS_EXTERNAL . 'billpay/utils/billpay_status_requests.php') && defined('MODULE_PAYMENT_BILLPAY_STATUS') && MODULE_PAYMENT_BILLPAY_STATUS == 'True') {
          require_once(DIR_FS_EXTERNAL . 'billpay/utils/billpay_status_requests.php');
      }
      // EOF - Fallback for shop version < 3.0.0
